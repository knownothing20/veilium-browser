...
5. **Corrective — First Exact Reviewed Provider Path**
   - add one pinned official Windows x64 Chromium Snapshot Provider;
   - freeze archive and executable identity, source, license/provenance, limitations, and rollback rules;
   - run the same exact binary through M4.2, M4.3, and M4.4 Evidence;
   - return to Phase 4 Closing Review after merge.

The full scope, non-scope, platform policy, acceptance criteria, rollback rules, and exit gate are defined in `docs/PHASE_04.md`. Logical data and evidence contracts are defined in `docs/PHASE_04_CONTRACTS.md`. The corrective boundary is frozen in `docs/PHASE_04_CORRECTIVE_PLAN.md`.
