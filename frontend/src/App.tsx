...
export default function App() {
  ...
  const adapters = () => <><Heading eyebrow="Managed external runtimes" title="Proxy adapters" description="Register local Xray or sing-box binaries with version, source, license and integrity metadata. Reviewed profiles run through supervised loopback SOCKS5 runtimes; automatic downloads remain disabled." /><AdapterRegistry records={data.adapters} nativeMode={backend.isNative()} busy={adapterBusy} error={adapterError} onPick={pickAdapter} onImport={importAdapter} onVerify={verifyAdapter} onDelete={removeAdapter} /></>
