...
func runTestKernelProbe() {
	...
	path := "/devtools/browser/test-kernel-probe"
...
func TestImportRejectsExecutableThatCannotStartAsChromium(t *testing.T) {
	root := t.TempDir()
	source := root + string(os.PathSeparator) + "broken-chromium.exe"
	if err := os.WriteFile(source, []byte("not an executable"), 0o700); err != nil {
		t.Fatal(err)
	}
	store, err := Open(root+string(os.PathSeparator)+"kernels.json", root+string(os.PathSeparator)+"managed")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := store.Import(ImportRequest{Name: "Broken", Provider: "custom-chromium", Version: "148.0.0", SourcePath: source}); err == nil {
		t.Fatal("expected launch probe rejection")
	}
}
