...
func TestApplyWaitsForInitialPageTarget(t *testing.T) {
	listener, err := net.Listen("tcp4", "127.0.0.1:0")
	if err != nil {
		t.Fatal(err)
	}
	...
	mux.HandleFunc("/json/list", func(writer http.ResponseWriter, _ *http.Request) {
		...
		_, _ = fmt.Fprint(writer, `[{"id":"page-after-ready","type":"page"}]`)
	})
	mux.HandleFunc("/devtools/browser/test", func(writer http.ResponseWriter, request *http.Request) {
		...
		for count := 0; count < 3; count++ {
			...
			if message.Method == "Browser.getWindowForTarget" {
				result = map[string]any{"windowId": 8}
			} else if message.Method == "Browser.getWindowBounds" {
				result = map[string]any{"bounds": map[string]any{"width": 1280, "height": 800, "windowState": "normal"}}
			}
			encoded, _ := json.Marshal(result)
			if err := connection.WriteJSON(cdpResponse{ID: message.ID, Result: encoded}); err != nil {
				return
	...
	state, err := controller.Apply(context.Background(), port, "ws://127.0.0.1:"+strconv.Itoa(port)+"/devtools/browser/test", domain.WindowPlan{Width: 1280, Height: 800, DeviceScaleFactor: 1})
	if err != nil {
		t.Fatal(err)
	}
	if !state.Applied {
		t.Fatalf("managed window was not applied: %#v", state)
	}
	mu.Lock()
	defer mu.Unlock()
	if listCalls < 3 {
		t.Fatalf("expected target discovery retries, got %d calls", listCalls)
	}
}
